...
useEffect(() => {
  ...
  const [preview, setPreview] = useState(state?.imagen_url || "");
  ...
  const [carreras, setCarreras] = useState(
    libro.carrera ? libro.carrera.split(", ") : [],
  );
  ...
  const handleChange = (e) => {
    ...
    /* SOLO LETRAS */
    if (name === "autor" || name === "ciudad" || name === "facultad") {
      if (name === "autor") {
    ...
    /* SOLO NUMEROS */
    if (name === "cantidad_ejemplar" || name === "anio_publicacion") {
  ...
  const validarFormulario = () => {
    ...
    if (!libro.titulo?.trim()) {
      nuevosErrores.titulo = "Ingrese el título";
    } else if (libro.titulo.length < 3) {
      nuevosErrores.titulo = "Mínimo 3 caracteres";
    }

    /* AUTOR */
    if (!libro.autor?.trim()) {
      nuevosErrores.autor = "Ingrese autor";
    }

    /* EDITORIAL */
    if (!libro.editorial?.trim()) {
      nuevosErrores.editorial = "Ingrese editorial";
    }

    /* AÑO */
    if (!libro.anio_publicacion) {
      nuevosErrores.anio_publicacion = "Ingrese el año";
    } else if (
      libro.anio_publicacion.length < 4 ||
      libro.anio_publicacion.length > 4
    ) {
      nuevosErrores.anio_publicacion = "Debe tener 4 dígitos";
    } else if (
      parseInt(libro.anio_publicacion) < 1900 ||
      parseInt(libro.anio_publicacion) > new Date().getFullYear()
    ) {
      nuevosErrores.anio_publicacion = "Año inválido";
    }

    /* EJEMPLARES */
    if (!libro.cantidad_ejemplar) {
      nuevosErrores.cantidad_ejemplar = "Ingrese cantidad";
    } else if (parseInt(libro.cantidad_ejemplar) < 0) {
    ...
    if (!libro.ciudad?.trim()) {
      nuevosErrores.ciudad = "Ingrese ciudad";
    }

    /* FACULTAD */
    if (!libro.facultad?.trim()) {
      nuevosErrores.facultad = "Ingrese facultad";
    }

    /* CARRERA */
    if (carreras.length === 0) {
      nuevosErrores.carrera = "Seleccione carrera";
    }
      ...
      <div className="form-modificar">
        ...
        <div className="form-row-2">
          ...
          <div className="campo">
            ...
            {errores.editorial && (
              <span className="error">{errores.editorial}</span>
            )}
          </div>
          ...
          <div className="campo">
            ...
            {errores.anio_publicacion && (
              <span className="error">{errores.anio_publicacion}</span>
            )}
          </div>
        </div>
        ...
        <div className="form-row-3">
          ...
          <div className="campo">
            ...
            <input
              ...
              value={libro.cantidad_ejemplar || ""}
              onChange={handleChange}
            />

            {errores.cantidad_ejemplar && (
              <span className="error">{errores.cantidad_ejemplar}</span>
            )}
          </div>
          ...
          <div className="campo">
            ...
            {errores.facultad && (
              <span className="error">{errores.facultad}</span>
            )}
          </div>
        </div>
        ...
        {errores.submit && <p className="submit-errorM">{errores.submit}</p>}
