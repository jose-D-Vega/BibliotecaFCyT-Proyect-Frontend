...
function PrestamoCard({ prestamo, onAccion }) {
  ...
  const fechaSolicitud = new Date(prestamo.fecha_solicitud).toLocaleDateString('es-PY', {
    day: '2-digit', month: '2-digit', year: 'numeric'
  })

  const fechaTope = prestamo.fecha_tope_devolucion
    ? new Date(prestamo.fecha_tope_devolucion).toLocaleDateString('es-PY', {
        day: '2-digit', month: '2-digit', year: 'numeric'
      })
    : null
  ...
  return (
    <>
      <div className="prestamo-card">
        ...
        {(mostrarFechaTope || esCancelable || esRenovable) && (
          <div className="prestamo-card__footer">
            {mostrarFechaTope && fechaTope && (
              <span className={`prestamo-card__fecha-tope ${estaVencido ? 'vencido' : ''}`}>
                {prestamo.estado_prestamo === 'devuelto'
                  ? `Devuelto — fecha límite era ${fechaTope}`
                  : estaVencido
                    ? `Venció el ${fechaTope}`
                    : `Devolver antes del ${fechaTope}`
                }
              </span>
            )}
      ...
      {modal === 'cancelar' && (
        <div className="prestamo-modal-overlay">
          <div className="prestamo-modal-box">
            ...
            {error && <p style={{ color: '#f87171', marginBottom: '12px', fontSize: '0.875rem' }}>{error}</p>}
            <div className="prestamo-modal-acciones">
              <button
      ...
      {modal === 'renovar' && (
        <div className="prestamo-modal-overlay">
          <div className="prestamo-modal-box">
            <h3>Solicitar renovación</h3>
            <p>
              Se enviará una solicitud de renovación. El bibliotecario
              deberá aprobarla antes de que venza el plazo actual.
            </p>
            {error && <p style={{ color: '#f87171', marginBottom: '12px', fontSize: '0.875rem' }}>{error}</p>}
            <div className="prestamo-modal-acciones">
              <button
