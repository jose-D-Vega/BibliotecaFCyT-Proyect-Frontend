...
function HistorialPrestamos() {
  ...
  const cargarHistorial = async () => {
    ...
    try {
      ...
      // Filtrar por fecha en el frontend (el backend no tiene ese filtro aún)
      let data = res.data
      if (fechaDesde) {
        data = data.filter(p =>
          new Date(p.fecha_solicitud) >= new Date(fechaDesde)
        )
      }
      if (fechaHasta) {
        data = data.filter(p =>
          new Date(p.fecha_solicitud) <= new Date(fechaHasta + 'T23:59:59')
        )
      }

      // Traer detalles de cada préstamo
      const conDetalles = await Promise.all(
        data.map(p => getLoanById(p.id_prestamo))
      )
      setPrestamos(conDetalles.map(r => r.data))
    } catch (err) {
      console.error('Error al cargar historial:', err)
  ...
  return (
    <div>
      <div className="historial-filtros">
        ...
        {(estado || fechaDesde || fechaHasta) && (
          <button
            className="prestamo-btn cancelar"
            onClick={() => {
              setEstado('')
              setFechaDesde('')
              setFechaHasta('')
