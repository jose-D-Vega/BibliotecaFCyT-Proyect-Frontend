...
function PrestamoDetalleEjemplares({ materiales, detalles, estado }) {
  function getDetalle(id) {
    return detalles.find((d) => d.id_ejemplar === id);
  }
      ...
      {materiales.map((material, i) => (
        <div key={i} className="prestamo-material-item">
          ...
          <div className="prestamo-material-item__copies">
            {(material.ejemplares || []).map((id) => {
              const detalle = getDetalle(id);

              const estadoEjemplar =
                detalle?.estado_prestamo_ejemplar?.toLowerCase();
                  ...
                  {estado?.toLowerCase() === "devuelto" &&
                    detalle?.fecha_devolucion && (
                      <div className="prestamo-copy-return">
                        <small>Se devolvió en</small>

                        <strong>{formatFecha(detalle.fecha_devolucion)}</strong>
                      </div>
                    )}

                  {estadoEjemplar && (
                    <div
                      className={`prestamo-copy-status prestamo-copy-status--${estadoEjemplar}`}
                    >
