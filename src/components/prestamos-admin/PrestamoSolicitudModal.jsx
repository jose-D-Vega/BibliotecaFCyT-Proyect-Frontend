...
function PrestamoSolicitudModal({ open, onClose, solicitud, onAceptar }) {
  ...
  const allCopies = useMemo(() => {
    return (
      solicitud?.materiales?.flatMap((material) =>
        material.ejemplares.map((ejemplar) => ejemplar),
      ) || []
    );
  }, [solicitud]);
  ...
  function handleConfirm() {
    onAceptar({
      ...solicitud,
      selectedCopies,
      modo: isReserva ? "RESERVA" : isRenovacion ? "RENOVACION" : "PRESTAMO",
    });
  }
          ...
          <div>
            ...
            <p>
              {solicitud.esRondaFinalReserva ? (
                "Reserva lista para entregar — todos los ejemplares ya fueron devueltos"
              ) : (
                <>
                  Solicitud de{" "}
                  {isReserva
            ...
            {
              solicitud.esReservaConDecisionPendiente &&
              (solicitud.yaAprobados > 0 || solicitud.yaRechazados > 0) && (
                <p className="solicitud-modal__aviso-parcial">
                  Esta reserva ya tiene una decisión previa: {solicitud.yaAprobados} ejemplar(es) aprobado(s)
                  y {solicitud.yaRechazados} rechazado(s). Acá solo podés decidir sobre lo que sigue pendiente.
                </p>
              )
            }
          </div>

          <button className="solicitud-modal__close" onClick={onClose}>
            ✕
          </button>
        </div>
