...
import { getSancionesComportamientoByUsuario } from '../../services/sanctions.services'
...
const ModalSancionesComportamiento = ({ grupo, onCerrar, onActualizar }) => {
  ...
  const fetchSanciones = async () => {
    try {
      setLoading(true)
      const data = await getSancionesComportamientoByUsuario(grupo.id_usuario)
      setSanciones(data)
    } catch {
    ...
    <div className="modal-sancion-overlay" onClick={onCerrar}>
      <div className="modal-sancion-box" onClick={e => e.stopPropagation()} style={{ maxWidth: 640 }}>
        ...
        <div className="modal-sancion__body">
          {loading && (
            <p style={{ color: 'rgba(255,255,255,0.4)', textAlign: 'center' }}>Cargando...</p>
          )}
